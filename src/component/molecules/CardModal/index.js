...
export default function CartModal(props) {
  ...
  return (
    <div>
      ...
      >
        <Modal.Header className="CartModal_header container" closeButton></Modal.Header>
        <Modal.Body className="row">
          <div className="col-12 col-md-5 img">
            <div
              className="img_top mb-2"
            >
              <img
                className=" mb-3"
                height={200}
              ...
              {props.dataImg &&
                props.dataImg.map((item, index) => {
                  return (
                    ...
                    >
                      {" "}
                      <img className="py-1" width={45} height={45} src={item.src} alt="s" />
                    </button>
                  );
                })}
            </div>
          ...
          <div className="col-12 col-md-7">
            ...
            <div>{props.dataRam && props.dataRam.map((item, index) => {
              return (
                <button onClick={() => handleRam(index)} className={`CartModal_Ram ${index === activeRam ? "activeRam" : ""}`}>
                  <span>{item}</span>
                  <img alt="img" className={`CartModal_RamImg ${index === activeRam ? "d-block" : "d-none"}`} src="https://frontend.tikicdn.com/_desktop-next/static/img/pdp_revamp_v2/selected-variant-indicator.svg" />
                </button>
              );
            })}
            </div>
            <div className="d-flex">{dataColor.map((item, index) => {
              return (
                <button style={{ background: item }} onClick={() => handleColor(index)} className={`CartModal_color ${index === activeColor ? "activeColor" : ""}`}>
                </button>

              );
            })}
            </div>
            <p>Style Code Live is a daily, live show where style enthusiasts can connect, chat, shop, and get the inside scoop on the latest fashion and beauty trends.</p>
            <Button
              className="btnheart"
            ...
            >
              <AiOutlineShoppingCart
                style={{ fontSize: "28px", color: "#fbda00" }}
                className=""
              />{" "}
            </Button>
            <SocialShare
